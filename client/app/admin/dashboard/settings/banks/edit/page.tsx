...
import { ArrowLeft, Save, Loader2, IndianRupee, X, Image as ImageIcon } from "lucide-react";
import { useUpdateAdminBankDetailMutation, useGetAdminBankDetailsQuery } from "@/redux/apies/paymentApi";
...
export default function EditBankPage() {
    ...
    // Fetch list to find the item
    // Ideally we would have a getSingleBank endpoint
    const { data: paymentData, isLoading: isLoadingData } = useGetAdminBankDetailsQuery(adminUser?.id as number, {
        skip: !adminUser?.id,
    });

    // Update mutation
    const [updateBank, { isLoading: isUpdating }] = useUpdateAdminBankDetailMutation();

    const [qrPreview, setQrPreview] = useState<string | null>(null);
    const [qrFile, setQrFile] = useState<File | null>(null);

    const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm<BankFormValues>({
        resolver: zodResolver(bankSchema),
    });

    useEffect(() => {
        if (paymentData?.data && id) {
            const banks = Array.isArray(paymentData.data) ? paymentData.data : [paymentData.data];
            ...
            if (bank) {
                reset({
                    ...
                    usdt_address: bank.usdt_address || "",
                    usdt_network: bank.usdt_network || "TRC20",
                ...
                if (imgPath) {
                    ...
                    setQrPreview(fullSrc);
                }
            }
        }
    }, [paymentData, id, reset]);
    ...
    const onSubmit = async (data: BankFormValues) => {
        ...
        try {
            ...
            await updateBank({ id: Number(id), data: formData }).unwrap();
    ...
    if (!id) {
        return <div className="p-10 text-center text-red-500">Invalid Bank ID</div>;
    }
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold bg-linear-to-r from-primary to-primary/60 bg-clip-text text-transparent">Edit Bank Account</h1>
                    <p className="text-muted-foreground text-sm">Update the details for the bank account.</p>
                </div>
            </div>
