...
import { ArrowLeftRight, Search, Filter, Check, X, Eye, Download, FileText } from "lucide-react";
import { useGetAllAdminDepositsQuery, useUpdateAdminDepositStatusMutation } from "@/redux/apies/depositApi";
...
export default function DepositsPage() {
    ...
    // API Hooks
    // Note: providing void to hook to match signature if no params needed initially, or empty object
    const { data: deposits, isLoading, isFetching, refetch } = useGetAllAdminDepositsQuery();
    const [updateStatus, { isLoading: isUpdating }] = useUpdateAdminDepositStatusMutation();
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold bg-linear-to-r from-primary to-amber-600 bg-clip-text text-transparent">Deposit Requests</h1>
                    <p className="text-muted-foreground text-sm">Manage and review all user deposit requests.</p>
                </div>
            </div>
                            ...
                            {(isLoading || isFetching || isUpdating) ? (
                                ...
                                <>
                                    {filteredDeposits?.map((deposit: Deposit) => (
                                        <tr key={deposit.id} className="hover:bg-muted/30 transition-colors border-b border-border">
                                            <td className="px-6 py-4">{deposit.id}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="font-medium text-foreground">{deposit.user?.name || "Unknown"}</span>
                                                    <span className="text-xs text-muted-foreground">{deposit.user?.email}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 font-mono font-medium text-foreground">
                                                ₹{parseFloat(deposit.amount.toString()).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                            </td>
                                                ...
                                                {deposit.receipt ? (
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <button
                                                                onClick={() => handleViewImage(getImageUrl(deposit.receipt!)!)}
                                            ...
                                            <td className="px-6 py-4 text-muted-foreground">
                                                <FormattedDate date={deposit.created_at} />
                                                <div className="text-[10px]"><FormattedDate date={deposit.created_at} options={{ hour: 'numeric', minute: 'numeric', second: 'numeric' }} /></div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium border ${deposit.status === 'approved' ? 'bg-green-500/10 text-green-600 border-green-500/20' :
                                                    deposit.status === 'rejected' ? 'bg-red-500/10 text-red-600 border-red-500/20' :
