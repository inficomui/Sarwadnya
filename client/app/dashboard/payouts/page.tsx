...
export default function PayoutsPage() {
    ...
    const totalAmount = payouts.reduce((sum, item) => sum + Number(item.amount), 0);
    const totalRoi = payouts.filter(p => p.type === 'roi').reduce((sum, item) => sum + Number(item.amount), 0);
    const totalReferral = payouts.filter(p => p.type === 'referral').reduce((sum, item) => sum + Number(item.amount), 0);
